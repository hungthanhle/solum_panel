...
//================ response data html ===================//
include "../../helpers/06-03-report-data-total-panel/response.php";
	...
	case 'select':
		...
		if($machine == 'ELECTRIC_PANEL')
		{
			...
			while( $area_machine_tag = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC) ) {
				...
				$area_machine_tags[$area][$key_name] = array_fill_keys($months, "--");
			...
			$sql = "SELECT
					...
					AND [factory] = '$factory'
					AND [machine] = '$machine'
					AND [ymd] >= '{$year}0101' and [ymd] <= '{$year}1231'
			...
			while( $area_machine_tag = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC) ) {
				...
				if (isset($join_conditions[$hash_key])) {
					$area_machine_tags[$area][$key_name][$month] = $total;
				}
			}
			sqlsrv_free_stmt($stmt);
			...
			*/
			...
			//--------------------------- response html data -------------------------------//
			$html = html_loading_data($months, $area_machine_tags);
			//--------------------------- response html data -------------------------------//
